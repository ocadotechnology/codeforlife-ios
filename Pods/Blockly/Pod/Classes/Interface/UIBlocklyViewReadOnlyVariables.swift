...
extension UIBlocklyView {
    ...
    weak var nextBlockly: UIBlocklyView? {
        return blockly.nextConnection?.targetConnection?.sourceBlockly.blocklyView as? UIBlocklyView
    }
    ...
    weak var parentBlockly: UIBlocklyView? {
        if let previousTargetConnection = blockly.previousConnection?.targetConnection {
            return previousTargetConnection is InputStatementConnection ?
                previousTargetConnection.sourceBlockly.blocklyView as? UIBlocklyView:
                (previousTargetConnection.sourceBlockly.blocklyView as? UIBlocklyView)?.parentBlockly
        }
        return nil
    }
